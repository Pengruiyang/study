# 如何设计一个项目
   ...
   ## 技术层面
   1. 构建项目,创建项目的基本结构
   2. 引入必要的插件,路由、状态管理、ui库、http请求
   3. 其他常用工具 vueuse
   4. 代码规范: prettier、eslint
   5. 提交规范: husky,lint-staged
   6. 常见目录结构
# 介绍自己的项目
## 项目背景
...
# 桌面端优化
初期项目集成阶段,发现高频行情cpu占比150%的情况,发现是node层行情中心在热门股热门时间段交易量过大,导致频繁设置数据.在行情中心用了节流操作(300ms拉取一次买卖点数据).初期由于将部分业务组件的计算放在了store层,导致store层和render层会触发两次计算,修改后cpu降回了80%.内部有很多频繁赋值的代码,且保留了引用导致外部列表无法检测到变化.内部使用computed进行很多值的获取,但是发现在部分简单计算的时候使用,都会重新计算立即求值.(缓存结果,惰性求值,computed依赖的值没有变化,但是他依赖值也是个computed内部值发生变化导致全部重新计算).
根据热点图查看函数耗时进行优化.
...
数据: 
价值评估(根据不同的收益法进行收益计算)、
浙江数据知识产权一体化服务平台—数知通、(依法收集、经过一定算法加工、具有使用价值和治理成果属性的数据。
产品形态包括官网、企业后台、管理后台，实现面向数据知识产权领域的登记全流程业务和运用服务。
)
数据知识产权运营服务功能、
数据备案、数据交易、
哈希计算工具、
...
# 
数知通:
近年来，浙江聚焦数据权属不清晰、数据创新利用不充分、 数据权益保护举证难等问题，以数据知识产权制度改革为切入 点，初步形成了“存证+登记+运用+保护”“制度+技术”的数 据知识产权价值创造和价值实现路径，在推动构建权属明晰、 源头可溯、运营合规、治理系统的数据知识产权保护制度，深化数据知识产权登记运用实践等方面取得一定成效。为进一步 打通数据知识产权登记、运用、保护全链条，放大国家数据知 识产权地方改革试点成效，加快形成数据知识产权生态体系， 为确立数据知识产权制度在国家数据要素基础制度中的核心 地位贡献浙江力量，利用杭州“中国数谷”、中国（温州）数安港等数据领域重大平台，先行构建数据知识产权全链条体系，全方位推进数据知识产权保护和运用，打造数据知识产权生态试验区。


| 术语 | 说明 |
|---|---|
| 数据知识产权 | 依法收集、经过一定算法加工、具有使用价值和治理成果属性的数据。 |
| 申请主体 | 提出数据知识产权登记服务申请的应当是依法依规处理数据的单位或个人。合作处理数据的，应当共同提出登记服务申请。接受他人委托处理数据的，可以根据协议由委托方或双方共同提出登记服务申请。 |
| 三数一链 | 数据交易场所+数联网+“数据发票”+‌区块链跨链互认机制 |
| 数据发票 | “数据发票”即数据合规流通数字证书，用于实现数据确权交易全链路的标准化和工具化，解决数据流通全链路的合规问题。它是数据交易中的重要凭证，确保数据交易的合法性和透明性。 |
| 数据交易场所 | 数据交易场所是合法的数据交易空间，承担数据流通的延伸监管和服务功能。杭州目前有两个主要的数据交易所：浙江大数据交易中心（浙数交）和杭州数据交易所（杭数交）。浙数交主要在浙江省内拓展业务，而杭数交则更侧重于杭州市内的数据交易服务。 |


官方门户网站：提供政策宣传、业务介绍、样例模板、信息公示等公共服务功能
企业后台：为企业用户提供服务法务、登记申请、权益维护等一站式在线服务
管理后台：为政府和监管部门提供内容发布、数据统计、业务备案等管理功能
数据可视化大屏：实时展现数据知识产权登记情况、运用情况数据、整体态势等核心指标
