...
> 客户端主动发起SYN(同步序列编号 ) syn = i
  服务端收到并返回 SYN已经 ACK (确认字符 i+ 1)客户端的 SYN:j
  客户端接收到服务端的 SYN 和 ACK 后,发送 ACK 的 ACK(j+1) 给服务端,服务端收到后建立连接 
...
9. 数据传输完成, 判断 tcp 链接是否有 connection : keep-alive 字段保持链接 没有 则 tcp 四次挥手
  >**客户端发送 FIN(结束指令) 给服务端,用来关闭客户端到服务器的数据传送.客户端进入FIN-WAIT-1状态**
  a => b 我要断开链接  主动关闭 进入半终止状态
  **服务器接收到 FIN 后发送 ACK 给客户端.服务端进入close-wait状态**
  b => a 接受到关闭信息 b 也准备断开链接
  **客户端接收到ACK后进入FIN-WAIT-2状态,等待服务端信息传输完成**
  **服务器全部数据传输完成后关闭客户端连接后,发送一个 FIN 给客户端.服务端进入last-ack状态**
  b => a b 我断开链接了
  **客户端收到后,ACK 报文确认给服务端,服务端关闭.服务端进入close状态**
  a => b 好吧你关闭 ,我也关闭了
  **客户端等待 2msl 时间断开链接 2msl 主要怕最后一个包对面没有接收到客户端进入close状态**
...
14. js引擎解析过程: 调用js引擎执行js代码
    * 创建window对象.全局执行环境
    * 加载文件.完成js引擎,分析语法词法是否合法,合法进入预编译.
    * 预编译.浏览器寻找全局变量申明,把它作为window属性加入window对象中,赋予初始值undefined.
    * 解释执行.给变量赋值.函数执行,将函数的环境推入一个环境的栈中,执行完成后再弹出.控制权交还给之前的环境.
