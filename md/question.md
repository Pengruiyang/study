...
preload: 当前浏览中预先加载其他的资源但不阻塞初步加载
preconnect: 用户可能需要来自目标域名的资源,抢先启动与该域名的链接
dns-prefetch: 域名预解析
prefetch: 浏览器用户未来会用到哪些资源,提前获取
...
也是针对代码处理的一系列工具链,把代码当成的字符串处理,并不运营代码,包括编译构建、静态分析、格式化、CI/CD等等.
编译构建: 分析模块之间的依赖关系,从入口模块开始构建依赖图,对其中每个节点分别用对应的编译器处理
# 页面加载慢问题定位
1. newwork 资源请求
...
# 前端路由直接刷新404怎么处理
后端进行配置
1. nginx配置try files,前端自定义404页面控制返回

# SSR和SSG的区别
SSR: 对应的服务端同样是将已经组合好的HTML文档直接返回给客户端,客户端不需要下载js文件就能渲染出整个页面.
SSG: react官网使用技术,与SSR相同之处就是对应的服务端同样将组合好的html文档返回给客户端.不同之处在于,每个页面对应的HTML文档在项目build打包构建时就已经生成好了,用户请求的时候服务端不需要在发送其他请求和进行二次封装,直接返回html文档,客户端与服务端通信更加简单.但是每次变更内容时都需要重新构建和部署应用,不适用制作经常变更内容(也不需要权限控制)的网站.