# nodejs事件
node.js 是单进程单线程应用程序,但是 v8 引擎提供的异步执行回调接口,通过这些接口可以处理大量的并发,所以性能非常高

# 如何理解 node 单线程
Node 最核心的就是 v8 引擎,node 启动后,会创建 v8 实例.这个实例是多线程的.包含
主线程: 编译、执行代码
编译/优化线程: 主线程执行时,优化代码
分析器
...
老生代标记清除,清理过程后,可能会出现内存不连续的状态.对后续内存分配问题.会进行标记整理
...
# node 分布式改造
应用状态被应用的某个节点进行了单独的维护,导致系统对外无法形成有效的整体.解决方案就是将这样的状态分布到应用集群外,形成一个独立的、公共的服务.可以采用 redis 作为分布式缓存维护一个任务队列,各应用节点收到请求后先将任务发送到任务队列,所有节点再根据自身任务情况拉取任务,形成一个生产/消费模型,每个应用节点都作为生产者向任务队列生产任务,同时也作为消费者从任务队列拉取任务进行消费.
